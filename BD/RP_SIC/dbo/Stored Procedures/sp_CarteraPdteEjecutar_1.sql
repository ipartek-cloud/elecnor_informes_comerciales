CREATE PROCEDURE [dbo].[sp_CarteraPdteEjecutar]
	@Usuario varchar(50), 		
	@pAño int,
	@pMes int,
...
BEGIN
	...
	INSERT INTO #vwWEB_OFERTAS_CA(CodOferta,FAdjudicacion,DesOfer,ImporteTotal,Tipo) 	-- Insertamos Ofertas que No son Baja
	...
	WHERE (year(FAdjudicacion)<@pAño OR (year(FAdjudicacion)=@pAño AND month(FAdjudicacion)<=@pMes))
		   AND dbo.OfertasSQL.CodOferta IN (SELECT CodOferta FROM #Ofertas) 
...
-------------------------------- Hasta AQUI
	...
	BEGIN
		...
		FROM	(SELECT q.CodOferta, count(q.Obra) NumObras
					FROM	(SELECT CodOferta, Obra
								FROM WEB_CarteraDetalladaUsuarioCentro
								 WHERE Usuario = @Usuario
								 GROUP BY CodOferta, Obra
...
----------------------------------------------
		SELECT q.CodOferta, SUM(q.ImporteContratado) Contratacion, SUM(q.ImporteProduccion) Produccion, SUM(q.ImporteContratado)-SUM(q.ImporteProduccion) CarteraPendiente
							...
							,q.TotalObrasOferta
		from (
			select CodOferta, ImporteContratado, SUM(ImporteProduccion) ImporteProduccion, TotalObrasOferta 
			from WEB_CarteraDetalladaUsuarioCentro	
			where   Usuario = @Usuario
			group by CodOferta, ImporteContratado, TotalObrasOferta) q INNER JOIN #OfertaDatosAnuales_TMP t ON q.CodOferta=t.CodOferta
		group by q.CodOferta, q.TotalObrasOferta,t.Produccion_A, t.CostoTotal_A, t.MargenProduccion_A, t.PorcProduccion_A
