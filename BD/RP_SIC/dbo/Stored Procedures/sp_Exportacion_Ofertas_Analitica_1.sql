...
CREATE PROCEDURE [dbo].[sp_Exportacion_Ofertas_Analitica]
@pCentros varchar(max)=''
AS
BEGIN
PRINT DB_NAME() + '..Exportacion_Ofertas_Analitica'
	...
	CREATE TABLE #OfertasSQL (IdOferta varchar(10), IdRegularizacion int, Oferta varchar(1000), IdCliente varchar(8), Cliente varchar(1000), IdProvPais varchar(2), ImporteAdjudicado float, ImporteOfertado float, Tipo varchar(1), FechaAdjudicacion date, Causa varchar(100), Baja varchar(1), IdCentroE varchar(3), AA varchar(4), MM varchar(2))

	SET @Sql = 'SELECT CDOFT IdOferta, 0 IdRegularizacion, DCOF Oferta, CDCLI IdCliente, DESPRO Cliente, PROOF IdProvPais, PREAD ImporteAdjudicado, IMAOF ImporteOfertado, WS10 Tipo, CAST(AA+MM+DD as date) FechaAdjudicacion
	...
	SET @Sql = 'SELECT CDOFT IdOferta, NUMRE Idregularizacion, DCOF Oferta, CDCLI IdCliente, DESPRO Cliente, PROOF IdProvPais, IMPRE ImporteAdjudicado, 0 ImporteOfertado, WS10 Tipo, CAST(AA+MM+DD as date) FechaAdjudicacion
	...
	SET @Sql = 'SELECT CodOferta IdOferta, ISNULL(NumRegularizacion, 0) IdRegularizacion, DescripcionOferta Oferta, CodCliente IdCliente, '''' Cliente, CodProv IdProvPais, 
	...
	SELECT IdOferta, Oferta, RIGHT(REPLICATE('0',8)+IdCliente, 8) IdCliente, Cliente, IdProvPais, ISNULL(P.Pais, '') ProvPais,
			SUM(ImporteAdjudicado) ImporteAdjudicado, SUM(ImporteOfertado) ImporteOfertado, Tipo, MIN(FechaAdjudicacion) FAdjudicacion,  @pCentros CentrosAS400, RIGHT('000' + IdCentroE, 3) IdCentroE
	FROM (
			SELECT * FROM #Ofertas
			UNION
	...
	GROUP BY IdOferta, Oferta, RIGHT(REPLICATE('0',8)+IdCliente, 8), Cliente, IdProvPais, Tipo, IdCentroE, P.Pais
	ORDER BY IdOferta
