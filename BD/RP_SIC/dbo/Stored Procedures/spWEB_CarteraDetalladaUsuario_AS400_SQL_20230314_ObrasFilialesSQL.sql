...
-- exec spWEB_CarteraDetalladaUsuario_AS400_SQL 'JRZULAIKA_3107', 2019,5, 75, null
-- exec spWEB_CarteraDetalladaUsuario_AS400_SQL 'ELUQUE_3107', 2019,5, 75, null
-- select * from web_CarteraDetalladaUsuarioCentro  where usuario like 'JRZULAIKA%' and nombreobra like '335-00 av%' order by codoferta 


CREATE PROCEDURE [dbo].[spWEB_CarteraDetalladaUsuario_AS400_SQL_20230314_ObrasFilialesSQL]
	@Usuario varchar(50), 		
	@pAño int,
	@pMes int,	
...
---------------------------------------------------------------- hasta AQUÍ
	...
	-- NO son de Reparto: Mismo Importe
		...
		WHERE (year(FAdjudicacion)<@pAño OR (year(FAdjudicacion)=@pAño AND month(FAdjudicacion)<=@pMes)) AND dbo.OfertasSQL.CodCentro IN (SELECT CodCentro FROM #Centros) 
			  AND isnull(dbo.OfertasBajasSQL.CodOferta,0)=0 AND Reparto=0	
	...
	-- SI son de Reparto: Suma de los Importes
		...
		FROM  dbo.OfertasSQL LEFT JOIN dbo.OfertasBajasSQL ON dbo.OfertasSQL.CodCentro=dbo.OfertasBajasSQL.CodCentro AND dbo.OfertasSQL.CodOferta=dbo.OfertasBajasSQL.CodOferta
		WHERE (year(FAdjudicacion)<@pAño OR (year(FAdjudicacion)=@pAño AND month(FAdjudicacion)<=@pMes)) AND dbo.OfertasSQL.CodCentro IN (SELECT CodCentro FROM #Centros) 
			  AND isnull(dbo.OfertasBajasSQL.CodOferta,0)=0 AND Reparto=1 
	...
	FROM #WEB_CarteraUsuarioCentro_TMP
	      LEFT JOIN (
					...
					FROM (
							...
							WHERE Usuario=@Usuario 
								--AND CDOFT <> 1
								AND CDOFT NOT IN (1,2) -- Paco 01/12/2021 para incluir las obras Tipo 2 en el tratamiento de las obras Tipo 1
		  ...
		  ) vwTIPOUNO_Produccion_Detallado ON [#WEB_CarteraUsuarioCentro_TMP].CodOferta=vwTIPOUNO_Produccion_Detallado.CodOferta AND CAST([#WEB_CarteraUsuarioCentro_TMP].CodCentro AS INT)=CAST(vwTIPOUNO_Produccion_Detallado.CodCentro AS INT) 
...
-- Ahora se estaban excluyendo porque solo se consideraban ofertas adjudicadas hasta el mes en cuestion
	...
	) B on A.CodOferta=B.CodOferta AND CAST(A.CodCentro AS INT)=CAST(B.CodCentro AS INT) INNER JOIN 
	...
	FROM (
			...
			FROM ( 
					...
					WHERE Usuario=@Usuario 
						--AND CDOFT = 1
						AND CDOFT IN (1,2) -- Paco 01/12/2021 para incluir las obras Tipo 2 en el tratamiento de las obras Tipo 1
...
-------------------------------- Hasta AQUI
	...
	FROM WEB_CarteraDetalladaUsuarioCentro 
		INNER JOIN (
					SELECT id, IdOferta CDOFT, CentroCRM CTR, IdCentro+IdObra OBRA, '' OBRAL, Obra DSOBR, 
	...
	--INSERT INTO WEB_CarteraDetalladaUsuarioCentro(Usuario,Tipo,CodOferta,DescripcionOferta,FAdjudicacion,ImporteContratado,NombreObra,FApertura,FCierre,ImporteProduccion,ImporteFactura,ImporteFot,Est,CodCliente)
	SELECT 1, CodOferta CodOferta1, * FROM WEB_CarteraDetalladaUsuarioCentro WHere Usuario=@Usuario order by Tipo,CodOferta
	SELECT 2, IdOferta, id, IdOferta CDOFT, CentroCRM CTR, IdCentro+IdObra OBRA, '' OBRAL, Obra DSOBR, 
			RIGHT('00'+CAST(MONTH(FechaApertura) as varchar(2)), 2) + '_' + RIGHT(CAST(YEAR(FechaApertura) AS varchar(4)), 2) FAPERTURA,
			RIGHT('00'+CAST(MONTH(FechaCierre) as varchar(2)), 2) + '_' + RIGHT(CAST(YEAR(FechaCierre) AS varchar(4)), 2) FCIERRE,
			ProduccionOrigen SOP, FacturacionOrigen SOF, FOTOrigen SOL, Tipo TipoOferta
	-- , * 
	...
	SELECT WEB_CarteraDetalladaUsuarioCentro.Usuario, @Usuario,TipoOferta,CDOFT CodOferta,Isnull(DescripcionOferta,''),IsNull(FAdjudicacion,''),IsNull(ImporteContratado,0),TipoOferta+[dbo].[fnObra](TipoOferta, ObrasOtrasSQL.Obra,ObrasOtrasSQL.ObraL)  +  DSOBR, replace(ObrasOtrasSQL.FAPERTURA,'/','_'),replace(ObrasOtrasSQL.FCIERRE,'/','_'),SOP,SOF,SOL,Est,CodCliente
	FROM (SELECT * FROM WEB_CarteraDetalladaUsuarioCentro WHere Usuario=@Usuario AND CodOferta NOT IN ('1','2')) WEB_CarteraDetalladaUsuarioCentro 
			...
			--LEFT JOIN #OfertasDeBaja OB ON ObrasOtrasSQL.CDOFT = OB.CodOferta
	WHERE --WEB_CarteraDetalladaUsuarioCentro.Usuario IS NULL AND
			 ObrasOtrasSQL.CTR IN (SELECT CodCentro FROM #Centros)
			--AND OB.CodOferta is null
	GROUP BY WEB_CarteraDetalladaUsuarioCentro.Usuario, TipoOferta,CDOFT,DescripcionOferta,FAdjudicacion,ImporteContratado,TipoOferta+[dbo].[fnObra](TipoOferta, ObrasOtrasSQL.Obra,ObrasOtrasSQL.ObraL)  +  DSOBR, replace(ObrasOtrasSQL.FAPERTURA,'/','_'),replace(ObrasOtrasSQL.FCIERRE,'/','_'),SOP,SOF,SOL,Est,CodCliente
