...
CREATE PROCEDURE [dbo].[spContratacion_Mensual_ZONA_CLIENTE_90] 		
	@pAño int,
	@pMes int
	AS
BEGIN
	...
	WHERE  (year(FAdjudicacion)=@pAño ) AND month(FAdjudicacion) = @pMes AND CodSubDirGeneral=90
	...
	SELECT 3 as Orden,NombreDirNegocio as Nombre,dbo.[fnAgrupacionCentro](CodCentro,NombreCentro) as AgrupacionCentro,NomAgrupado +' ('+ dbo.fnPaises(Mercado,ClientesSQL.Pais) +')' as Cliente,DescripcionOferta, Sum(ImporteContratadoAcumulado) as ImporteContratadoMensual
	FROM @vContratacion 
	...
	--order by AgrupacionCentro,NombreDirNegocio, ImporteContratadoAcumulado desc
