CREATE procedure [dbo].[sp_SDG_Contratacion_Actividades_DN_20240904]
	@pAño as int=2022
AS
...
BEGIN
	...
	SET @sqlObjetivos = '
				SELECT  Ac.Orden, Obj.CodDDirNegocio, Obj.NombreDirNegocio, Ac.Agrupacion, Obj.CDAC1, Obj.Mercado,
						...
						0 Mes, Obj.Año
				FROM (
						SELECT Orden, Agrupacion, CDAC1,
								CASE WHEN CDAC1 IN (''04'', ''06'', ''09'')
								THEN CDAC1+CDAC2
								ELSE CASE WHEN CDAC1=''07'' THEN ''08'' ELSE CDAC1 END+''00''
								END CDAC
						FROM ActividadesSQL
