CREATE PROCEDURE [dbo].[spContratacion_DG_SDG_DN_SDNA_Ajuste] 		
	@pAño int,
	@pMes int
	AS
BEGIN
...
---------------------------------------------------------------- hasta AQUÍ
	...
	INSERT INTO @vContratacionMensualInfraEstructuras(CodSubDirGeneral,NombreSubDirGeneral,NombreDirNegocio,NombreSubDirNegocioArea,Pais,ImporteContratado,ImporteContratadoAcumulado,ImporteContratadoAcumuladoAñoanterior)		
	SELECT     0,'?', '?','?', dbo.Provincias.Pais, sum(dbo.OfertasSQL_Ajustes.Importe) as ImporteContratado,0,0
	...
	INSERT INTO @vContratacionMensualInfraEstructuras(CodSubDirGeneral,NombreSubDirGeneral,NombreDirNegocio,NombreSubDirNegocioArea,Pais,ImporteContratado,ImporteContratadoAcumulado,ImporteContratadoAcumuladoAñoanterior)		
	SELECT     0,'?', '?','?', dbo.Provincias.Pais, 0, sum(dbo.OfertasSQL_Ajustes.Importe) as ImporteContratado,0
	FROM         dbo.OfertasSQL_Ajustes INNER JOIN
                      dbo.Provincias ON dbo.OfertasSQL_Ajustes.CodProv = dbo.Provincias.CDPRO 
	WHERE AñoAdjudicacion=@pAño AND month(FAdjudicacion) <= @pMes
	GROUP BY Pais 
...
*/			
	...
	SELECT CodSubDirGeneral,NombreSubDirGeneral,NombreDirNegocio,NombreSubDirNegocioArea,Pais, sum(ImporteContratado) as ImporteContratado,Sum(ImporteContratadoAcumulado) as ImporteContratadoAcumulado, sum(ImporteContratadoAcumuladoAñoAnterior) as ImporteContratadoAcumuladoAñoAnterior 
