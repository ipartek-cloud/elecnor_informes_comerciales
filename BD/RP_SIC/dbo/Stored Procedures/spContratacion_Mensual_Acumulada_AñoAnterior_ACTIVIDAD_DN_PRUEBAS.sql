...
-- exec spContratacion_Mensual_Acumulada_AñoAnterior_ACTIVIDAD_DN_PRUEBAS 2017,4

CREATE PROCEDURE [dbo].[spContratacion_Mensual_Acumulada_AñoAnterior_ACTIVIDAD_DN_PRUEBAS] 		
	@pAño int,
	@pMes int
	AS
BEGIN
	...
	SELECT  CodCentro,CodAct1,CodAct2,
			...
			0
	FROM dbo.vwOFER 
	WHERE  (year(FAdjudicacion)=@pAño ) AND month(FAdjudicacion) <= @pMes and CodCentro=793
	...
	FROM         (SELECT dbo.vwREG.*  
				  FROM     dbo.vwREG
				  WHERE   (year(FAdjudicacion)=@pAño ) AND month(FAdjudicacion) <= @pMes  and CodCentro=793 ) AS vwRegularizacionesQ
	GROUP BY CodCentro,CodAct1,CodAct2
	...
	SELECT  CodCentro,codoferta,CodAct1,CodAct2,
			(dbo.fnImporteContratacion_MesActual(FAdjudicacion,@pAño,@pMes,ImporteContratado)) as ImporteContratado,
			(dbo.fnImporteContratacion_Acumulado(FAdjudicacion,@pAño,@pMes,ImporteContratado)) as ImporteContratadoAcumulado,
			0
	FROM         dbo.OfertasSQL 
	WHERE  (year(FAdjudicacion)=@pAño ) AND month(FAdjudicacion) <= @pMes  and CodCentro=793
	--GROUP BY CodCentro,CodAct1,CodAct2		
	...
	WHERE  Año=@pAño-1 AND Mes <= @pMes  and CodCentro=793
	GROUP BY CodCentro,CodAct1,CodAct2	
	

	SELECT CodDDirNegocio,NombreDirNegocio,sumarigrama.CodCentro,CodAct1,CodAct2,(ImporteContratadoAcumulado) as ImporteContratadoAcumulado, (ImporteContratadoAcumuladoAñoAnterior) as ImporteContratadoAcumuladoAñoAnterior 
	FROM (
	...
	WHERE sumarigrama.CodSubDirGeneral=221 and codact1='04' and NombreDirNegocio='Este'
