...
CREATE FUNCTION [dbo].[fnCart_DiferidaContratosSQL_Usuario_Anual_Trimestral] (@pUsuario  varchar(50))
RETURNS
...
BEGIN
  ...
  ELSE	-- solamante las Indicadas
	BEGIN
		...
		SELECT [ID],[Contrato],[Cliente],isnull([CodigoContratoClient],''),[Gerencia],isnull([Prorrogable],''),[Tipo],[Mercado],[FInicio],[FFinal],[FFinalEfectiva],isnull([Zona],'')
		FROM [dbo].[Cart_DiferidaContratosSQL]
		WHERE Vigente=1 AND Gerencia IN(SELECT Gerencia FROM [dbo].[WEB_UsuariosGerencias] WHERE Usuario=@pUsuario ) and Tipo<>'C'
	END
